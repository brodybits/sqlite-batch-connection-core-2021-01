...
- The API is designed to work with integer connection ID that is easy to use from C API or Java wrapper generated by GlueGen, or a component for a higher-level framework such as Apache Cordova or React Native.
- Possible to bind 64-bit integer, double-precision float, text string, or null values to SQL statemeents with `?` placeholders.
- Possible to retrieve one or multiple rows of results with 64-bit integer, double-precision floating point, text string, or null values.
- Possible to get last insert row ID and total number of changes which are needed to help match some of the capabilities of the *deprecated draft* Web SQL specification.
- Supports extremely simple, rudimentary error handling.

## Some known limitations

- Not tested with `Infinity`, `-Infinity`, or `NaN` values.
- In case of Apache Cordova, a helper plugin such as `cordova-sqlite-storage-file` (recommended) or `cordova-plugin-file` (recommended for external filesystem access on Android) should be used to resolve an absolute database file path before opening it. (Similar helper components would be recommended on React Native.)
